...
export class Memory {
    ...
    constructor() {
        ...
        //////////////////////////////////////////////////
        //////////TESTING//////////TESTING///////////////
        // this.addToMemory("whatever1", ".byte", [5, 6, 1]);

        // // alert("Memory Constructed. It is " + this.bytesArray.toString());
        // // alert("Currnet Index  is " + this.currentIndex);

        // this.addToMemory("whatever2", ".byte", [15, 16, 18, 11]);

        // this.addToMemory("whatever3", ".byte", [19, 9]);

        // this.addToMemory("whatever4", ".word", [32, 33, 43]);

        // alert("Memory Constructed. It is " + this.bytesArray.toString() + " number of bytes = " + this.bytesArray.length);
        // alert("The dictionary also works. here it is: " + this.dictionary.toString());
    ...
    storeWord(arrayName: string, index: number, byteValue: number) {
        if (index % 4 != 0) return; // throw error
        else {
    ...
    loadWord(arrayName: string, index: number): string {
        ...
        if (index % 4 != 0) return undefined; // throw error
        else {
            var offset: number = this.dictionary.getValue(arrayName) + index;

            var thingToReturn: string = "";

            // thingToReturn += this.bytesArray[offset] + this.bytesArray[offset - 1] + this.bytesArray[offset - 2] + this.bytesArray[offset - 3];
    ...
    storeByte(arrayName: string, index: number, byteValue: number) {
        ...
        // alert("this is the given array name " + arrayName + " given index " + index + " given byteValue " + byteValue);
        ...
        // alert("loadbyte offset is " + offset + " return this " + this.bytesArray[offset]);
    ...
    private goToNextByteIndex() {
        ...
        if (this.bytesArray[this.currentIndex] == "0") return;
        else {
    ...
    private goToNextWordIndex() {
        if ((this.currentIndex + 1) % 4 == 0) return;
        else {
