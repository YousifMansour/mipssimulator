...
export class MainController {
    ...
    constructor() {
        ...
        this.drawingObject = new DrawingClass(this.HTMLHandler, this.soundController);
        ...
        this.validator = new Validator(this.typeObjectArray, this.inputHandler, this.console);
        ...
        this.themeManager = new ThemeManager(this.HTMLHandler, this.drawingObject, this.mipsObject);
    ...
    updateColor() {
        ...
        if (this.HTMLHandler.changeColorButton.getAttribute("disabled") == "true") return;
    ...
    redrawFrames() {
        ...
        for (var i = 0; i < this.frameIndex; i++) {
            this.drawingObject.drawFrame(tempFramesArray[i],
    ...
    runAll() {
        // to make this work whgen going fullscreen, this.frameIndex needs to be incremented inside runInstruction method.
    ...
    private getNextFrame(): Function {
        ...
        var instrustionList: string[] = this.inputHandler.getInstructionsList(false);
    ...
    runNext() {
        ...
        if (!this.HTMLHandler.textArea.getAttribute("disabled")) this.HTMLHandler.hideTextArea();

        this.drawingObject.setAudio(true);

        this.drawingObject.drawFrame(this.getNextFrame(),
            this.getTypeObjecet(this.listInOrder[this.iterationIndex][0]).stringALU,
            this.getTypeObjecet(this.listInOrder[this.iterationIndex][0]).type,
            this.listInOrder[this.iterationIndex]);
    ...
    runPrevious() {
        ...
        if (this.frameIndex == 0) {
            if (this.iterationIndex - 1 >= 0) {
                ...
                this.frameIndex = this.getTypeObjecet(this.listInOrder[this.iterationIndex][0]).functionsArray.length; //-1
            }
    ...
    private updateButtons() {
        ...
        if (this.iterationIndex == 0 && this.frameIndex == 0) this.HTMLHandler.hidePreviousButton();

        else this.HTMLHandler.showPreviousButton();
        ...
        if (this.iterationIndex == this.listInOrder.length - 1 &&
            ...
            this.HTMLHandler.hideNextButton();
        else this.HTMLHandler.showNextButton();

        if (this.console.textarea.style.color == 'red') this.HTMLHandler.showEditButton();
